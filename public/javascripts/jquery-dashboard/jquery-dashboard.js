...
// Grab a query param by name, e.g. ?dashboard_id=2.
...
(function(jQuery) { // Create closure.
  ...
  jQuery.fn.dashboard = function(options) {
    ...
    dashboard.id = getParameterByName('dashboard_id');
    ...
    dashboard.saveColumns = function() {
      ...
      for (var c = 0; c < dashboard.columns.length; c++) {
        ...
        for (var w = 0; w < ids.length; w++) {
          ...
          params['columns[' + c + '][' + id + '][minimized]'] = (dashboard.widgets[id].minimized ? '1' : '0');
          params['columns[' + c + '][' + id + '][order]'] = w;
        }
      }

      // The ajaxCallback settings overwrite any duplicate properties.
      jQuery.extend(params, {dashboard_id: dashboard.id});
    ...
    dashboard.enterFullscreen = function(widget) {
      var element = widget.fullscreen.element;
      // Hide the columns.
      for (var c = 0; c < dashboard.columns.length; c++) {
      ...
      invokeCallback(opts.callbacks.enterFullscreen, dashboard, widget);
    };
    ...
    var dashboard_data = jQuery.extend({}, {dashboard_id: dashboard.id},
      opts.ajaxCallbacks.getWidgetsByColumn.data);

    jQuery.getJSON(opts.ajaxCallbacks.getWidgetsByColumn.url, dashboard_data, init);
    ...
    function init(widgets, status) {
      ...
      for (var c = 0; c < opts.columns; c++) {
        ...
        for (var w = 0; w < widgets[c].length; w++) {
          var info = widgets[c][w];
          var id = info.id;
    ...
    function widget(widget) {
      ...
      widget.showSettings = function() {
        if (widget.settings.element) {
          widget.settings.element.dialog({modal: true, width: 600});
      ...
      widget.hideSettings = function () {
        if (widget.settings.element) {
          widget.settings.element.dialog('close');
        }
        widget.settings.displayed = false;
      };
      widget.updateSettingsForm = function (eventData) {
        widget.saveSettings(eventData, true);
        return false;
      }

      widget.saveSettings = function (eventData, dryrun) {
        // Build list of parameters to POST to server.
        // serializeArray() returns an array of objects.  Process it.
        var params = widget.settings.element.serializeArray();
        ...
        widget.settings.innerElement.append(widget.throbber.show());
        ...
        params.push({name: 'dashboard_widget_id', value: widget.id});
        var method = 'POST';
        ...
        jQuery.ajax({
          type: method,
          url: opts.ajaxCallbacks.widgetSettings.url,
          data: params,
          success: function(response, status) {
            ...
            jQuery('.refresh-on-change, .subform input, .subform select', widget.settings.innerElement).change(widget.updateSettingsForm);
            ...
            if (widget.settings.displayed || dryrun) {
              // Ooops!  We had better take care of hiding the throbber and showing the settings form then.
              widget.throbber.hide();
              widget.showSettings();
            }

            invokeCallback(opts.widgetCallbacks.saveSettings, true);
            if (!dryrun) {
              // We cheat the settings state here, so double toggle.
      ...
      widget.enterFullscreen = function() {
        ...
        dashboard.enterFullscreen(widget);
      ...
      widget.reloadContent = function() {
        ...
        throbber = jQuery(opts.throbberMarkup).appendTo(widget.element);
      ...
      var params = jQuery.extend({}, opts.ajaxCallbacks.getWidget.data, {dashboard_widget_id: widget.id});
      jQuery.getJSON(opts.ajaxCallbacks.getWidget.url, params, init);
      ...
      function initSettings() {
        ...
        initThrobber();
        widget.element.find('.widget-content').hide();
        widget.settings = {
        ...
        var params = jQuery.extend({}, opts.ajaxCallbacks.widgetSettings.data,
        ...
        jQuery.getJSON(opts.ajaxCallbacks.widgetSettings.url, params, function(response, status) {
          ...
          // Bind the apply button's event handler.
          widget.settings.applyButton = jQuery('.widget-settings-apply', widget.settings.element).click(widget.updateSettingsForm);
          ...
          if (widget.settings.displayed) {
            ...
            widget.throbber.hide();
            widget.element.find('.widget-content').show();
            widget.showSettings();
          }
      ...
      function widgetSettingsHTML() {
        ...
        html += '    <input id="' + widget.id + '-settings-save" class="widget-settings-save" value="Ok" type="submit" />';
        html += '    <input id="' + widget.id + '-settings-apply" class="widget-settings-apply" value="Apply" type="submit" />';
